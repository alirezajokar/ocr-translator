...
async function applyBinding(slotPath, name, command, binding) {
  ...
  // `command` is itself a shell command line (main.js's selfInvokeCommand() wraps each
  // argument in single quotes for the shell that later executes it), so it contains
  // literal `'` characters. Passed bare, `gsettings set` tries to parse the value as
  // GVariant text syntax and chokes the instant it sees that leading quote (valid GVariant
  // string syntax has exactly one quoted token, not several). JSON.stringify's
  // double-quoted output is close enough to GVariant's own double-quoted string syntax to
  // pass through as a single string literal instead — same trick already used in
  // gnomeScreenshot.js for the same underlying reason.
  await runGsettings(['set', schemaWithPath, 'command', JSON.stringify(command)]);
 ...
 * string from settings and `captureCommand` as the shell command GNOME should run (the
 * caller computes this — see `selfInvokeCommand()` in main.js — so this module doesn't
 * need to know whether it's running from source or from an installed package). Also
 * cleans up the old Settings shortcut slot if it's still registered from before that
 * feature was removed. Throws on failure — caller decides how to surface that (e.g.
 * non-fatal toast, since the shortcut not being registered doesn't break anything else).
 */
async function applyShortcuts({ captureShortcut }, captureCommand) {
